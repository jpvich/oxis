...
use oxis_core::{EuropeanOption, MarketData, OptionType, OxisError, normal_cdf};
...
mod tests {
    ...
    fn floating_lookback_is_worth_more_than_vanilla() {
        ...
        let vanilla = crate::black_scholes(&opt(OptionType::Call, 100.0), &m).unwrap();
        assert!(
            lb > vanilla,
    ...
    fn fixed_lookback_dominates_vanilla() {
        ...
        let vanilla = crate::black_scholes(&opt(OptionType::Call, 105.0), &m).unwrap();
        assert!(
            lb > vanilla,

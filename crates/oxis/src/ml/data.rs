...
//! via [`crate::core::path_seed`], so a `(seed, n)` fixes the whole training set —
//! the same bit-reproducibility discipline as the Monte Carlo pricers.

use crate::core::{OptionType, OxisError, path_seed};
...
mod tests {
    use super::*;
    use crate::core::{EuropeanOption, MarketData};

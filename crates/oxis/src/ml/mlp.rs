...
//! seeded deterministically from [`crate::core::path_seed`] so a `(seed)` fixes the
//! whole model.
//!
...
//! [`crate::ml::train`]) backpropagates once more through this pass; the caches stored
//! here (`a`, `z`, `delta`, `tprime`) are exactly what that second pass consumes.

use crate::ml::activation::{softplus, softplus_prime};
use rand::Rng;

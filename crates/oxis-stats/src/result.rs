...
use crate::descriptive::{excess_kurtosis, mean, skewness, std_dev, variance};
use crate::drawdown::max_drawdown;
use crate::normality::jarque_bera;
use crate::relational::{autocorrelation, beta, correlation, covariance};
use crate::returns::{annualized_return, annualized_volatility, cumulative_return};
use crate::risk::{
    ...
    let (max_drawdown_v, max_drawdown_duration, calmar) = match req.prices {
        Some(p) => {
            ...
            (
                ...
                dd.map(|d| d.duration),
                crate::risk::calmar_ratio(p, ppy).ok(),
            )
        }
        None => (None, None, None),
    };

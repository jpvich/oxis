...
- **Pure-core principle.** Every module's logic lives in a pure, I/O-free core: plain functions over plain types, never touching files / stdout / stderr / `clap`. The CLI `run()`, the REPL, and the PyO3 bindings are thin wrappers around the *same* core.
- **One-way dependencies.** Module → core only. A module never imports another module's internals; shared logic belongs in the core.
...
A module is the unit of contribution. To add one, implement this fixed contract against the stable core — and touch nothing else:

1. **A pure, I/O-free core** — plain functions over plain types (e.g. `price(option, market, model) -> Result<PriceResult, OxisError>`). This is the part validated against QuantLib and wrapped by Python.
...
3. **A `run(args, ctx) -> anyhow::Result<()>`** thin wrapper for the CLI: parse args → build plain inputs → call the pure core → print via the output layer.
...
A module **must not**: import another module's internals, write progress to stdout (use stderr), or bypass the output layer. The `pricing` and `greeks` modules are the reference implementations — mirror their shape.

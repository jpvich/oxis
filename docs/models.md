...
**Deep Optimal Stopping.** Rather than learning a continuation *value*, DOS learns a
stopping *policy* directly: one network per exercise date emits a stop probability,
trained backward to maximise the expected discounted payoff of stopping now versus
following the already-learned downstream policy. The hard policy (`stop ⇔ F_n > ½`)
is applied to a fresh, independent path set — any admissible stopping time underprices
the American option, so out-of-sample evaluation yields a valid low-biased estimate.
The policy-gradient backprop is gated by a finite-difference gradient check. Both
engines' accuracy bands are sized for a deliberately light config (4096 paths, 10
exercise dates) to keep the suite fast; the discretization gap shrinks with more steps.

The same option priced four ways (binomial CRR ↔ classical LSM ↔ Deep LSM ↔ DOS) is
checked to agree within band in `tests/american_comparison_tests.rs`.

Multi-dimensional pricing surfaces (basket / Bermudan max-call), higher Greeks, and a
GPU backend are deferred to later Ring-4 milestones.

...
║    oxis-pricing · oxis-greeks · oxis-stats · oxis-ml (inference)   ║
...
core's trait contracts — never its internals.
...
[`oxis_core::contract`](../crates/oxis-core/src/contract.rs).
...
[`DataSource`](../crates/oxis-core/src/source.rs)), owns its client/cache/config,
and **confines all I/O to its own crate**. Consumers depend on the *trait*, not the
...
- **The core stays lean and runtime-agnostic:** no Polars/Arrow, no async runtime,
  no HTTP in `oxis-core`. (Enforced by review; quick check: `cargo tree -p
  oxis-core` shows only serde/serde_json/thiserror today.)
- **Polars is opt-in and local.** Heavy columnar machinery may be used *inside* the
  ...
  [`oxis_core::series`](../crates/oxis-core/src/series.rs) are the universal
  interchange that crosses crate boundaries; Polars is an internal compute detail.
...
| **long-term** | Market-data API | `oxis-data` | reserved (skeleton) |

The reserved crates exist today as **skeletons carrying their boundary contract**
